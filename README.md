...
| [`auditcore_harvest`](packages/auditcore_harvest) | 0.1.2 | Shared harvest core: source contracts, paging/retry engine, checkpoints and adapter contract tests | `auditcore_common==0.1.1`; Extras: `xml` | neu, gegen charakterisierte Verträge |
...
| [`auditcore_documents`](packages/auditcore_documents) | 0.3.3 | Characterized document comparison, German article-law synopsis and document pipeline core without web or database dependencies | `auditcore_common==0.1.1`; Extras: `docx`, `pdf-text`, `fuzzy`, `docx-render`, `pdf-render`, `mime`, `ocr-raster`, `donut`, `web`, `fastapi` | charakterisiert |
| [`auditcore_dummygenerator`](packages/auditcore_dummygenerator) | 0.1.2 | Framework-independent synthetic field and row generation | keine; Extras: `parallel` | charakterisiert |
| [`auditcore_entity_matching`](packages/auditcore_entity_matching) | 0.2.3 | Characterized entity name normalisation, LEI checks and transparent fuzzy matching | `auditcore_common==0.1.1`; Extras: `fuzzy` | charakterisiert |
...
| [`auditcore_invoicegenerator`](packages/auditcore_invoicegenerator) | 0.2.2 | Characterized synthetic invoice profiles and explicit test scenarios | `auditcore_dummygenerator==0.1.2`; Extras: `pdf` | charakterisiert |
| [`auditcore_invoicesynth`](packages/auditcore_invoicesynth) | 0.1.2 | Synthetische Trainings- und Testdaten für eine Donut-basierte Erkennung deutscher und österreichischer Rechnungen: Rechnungsbilder, Ziel-JSON, Manifest mit Datensatz-Hash und Bewertung. | `auditcore_invoicegenerator==0.2.2`; Extras: `render`, `train` | neu |
| [`auditcore_kanban`](packages/auditcore_kanban) | 0.1.1 | Framework-free Kanban domain: boards, rank keys, transitions, WIP limits, rights, events and a REST contract | keine; Extras: `ui`, `fastapi` | neu, gegen charakterisierte Verträge |
...
| [`auditcore_procurement`](packages/auditcore_procurement) | 0.2.3 | Procurement notice records (TED, HAD), import normalisation and versioned prechecks | `auditcore_common==0.1.1`; Extras: `html`, `sources` | charakterisiert |
...
| [`auditcore_risk`](packages/auditcore_risk) | 0.3.3 | Risk flags from explicit, versioned, source-bound rule profiles (legacy-exact riskanalysis and Flowstat red flags) | `auditcore_common==0.1.1`, `auditcore_entity_matching==0.2.3`; Extras: `fuzzy`, `pandas`, `procurement`, `web`, `fastapi` | charakterisiert |
| [`auditcore_sampling`](packages/auditcore_sampling) | 0.2.2 | Audit sampling sizes, selection and allocation with named method profiles | keine; Extras: `web` | charakterisiert |
| [`auditcore_statistics`](packages/auditcore_statistics) | 0.3.3 | Descriptive audit statistics (Benford) with named method profiles | `auditcore_common==0.1.1`; Extras: `web` | charakterisiert |
...
| [`auditcore_price_sources`](packages/auditcore_price_sources) | 0.1.2 | Preis- und Marktdatenquellen (Bundesbank, Destatis GENESIS, EIA, Tankerkönig, Overpass, EU Oil Bulletin) als Adapter auf `auditcore_harvest`, mit Einheit, Zeitbezug und Wertstatus je Datensatz. | `auditcore_harvest==0.1.2` | neu, gegen charakterisierte Verträge |
