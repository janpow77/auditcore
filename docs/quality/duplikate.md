...
| A16 | JSON-Objekt-Prüfung der REST-Verträge | identifiers `web.contract._object`, reporting `web.contract._object` (identisch); dazu beide `ContractError` (identisch mit `rest.ContractError`) | nur Fehlerklasse verschieden, Meldung „'<Pfad>' muss ein JSON-Objekt sein.“ gleich | `rest.json_object(value, path, error=)`, `ContractError` als Unterklasse von `rest.ContractError`; Differenztest `tests/test_rest.py`. **Abgeschlossen (Unreleased):** auch sampling `as_object` (Vorgabe „Anfrage“ bleibt im Paket), geo `_contract.Body.of` (eigener Standardcode `ungueltige_eingabe` als Unterklassen-Vorgabe) und extrapolation `_contract.Reader`; Differenztest gegen die wörtlichen Kopien in `tests/legacy_rest.py` |

## B. Wartet auf andere Arbeit (Kandidat, noch nicht zusammengeführt)

| # | Gruppe | Fundstellen | Befund | Warum wartend |
|---|---|---|---|---|
| B1 | REST-Schicht Stichprobe/Statistik | sampling/statistics `create_app` (identisch), `create_router`, `response`, `run`/`_analyse`, `get_profiles` | nach A15 verbleiben nur die Starlette-/FastAPI-Adapter (je eine Anweisung bzw. Routentabelle, unter der Gate-Schwelle) | bleibt im Paket: gebunden an Starlette/FastAPI, `auditcore_common` bleibt reine Standardbibliothek. geo ist umgestellt (Unreleased): `decode` → `decode_body(…, too_large_code="zu_gross", invalid_json_code="ungueltiges_json")`, `Reply`/`_json` → `rest.Reply`/`json_reply`; `create_app`/`routes` bleiben als Starlette-Adapter im Paket |
...
(Pflichtabhängigkeit `auditcore_identifiers==0.2.0`). Paritätstests je Paket
