...
fertig, TS-Spiegel in Arbeit), **UI geplant** (Oberfläche folgt mit
`@flowaudit/ui`), **Port** (bleibt bei der Anwendung, Schnittstelle vorhanden),
...
| Einstellungsdialog: Spalten hinzufügen („Neue Spalte“, max. 10), löschen (min. 1), per Drag-and-drop umsortieren, Label ändern, Farbe aus 8 Vorgaben oder eigenes Hex, Speichern nur bei Änderung und ohne leere Labels | UI `BoardSettings` + Core-Validierung | UI geplant |
...
| Board-Titel inline umbenennen (Klick, Enter/Blur, Esc bricht ab, leer → alter Titel) | Py `update_board(title=…)` (owner, edit); UI Kopfzeile | Py übernommen, UI geplant |
...
| Kartenfarbe, Hintergrundbild (Data-URI ≤ 2 MB im Frontend), `""` löscht | Py `color`/`image`; UI Farbwahl (10 Vorgaben, eigene Farbe), Textfarbe nach Luminanz | Py übernommen, UI geplant |
...
| Karte: Prioritätsstreifen, Kurzbeschreibung (80 Zeichen), max. 3 Tags + „+n“, Checklistenzähler, Frist (überfällig rot mit „!“, ≤ 3 Tage gelb), Dateizahl, Prompt-Stern, Alter (neu/h/d/w/mo), erledigt durchgestrichen | Py `deadline_state`; Core `ageLabel`, `cardView`; UI `KanbanCard` | Py Frist übernommen, Rest Core/UI |
| Detailpanel: Badge, Titel, Status (fest 3 Knöpfe), Priorität, Frist, Beschreibung (wächst mit), Tags, Checkliste, Kartendesign, Dateien, Prompt, Zeitstempel; Löschen | UI `CardDetail` (Status aus den Board-Spalten statt fest 3) | UI geplant |
...
| Drag-and-drop zwischen Spalten (vuedraggable) | UI mit MIT-Bibliothek oder eigener Umsetzung, Ablegen vor Karte | UI geplant |
...
| Fortschritt erledigt/gesamt mit Prozent, Mini-Balken je Spalte, Zähler | Py `board_stats`, `percent` (= `Math.round`) | übernommen; UI geplant |
| Seitenleisten-Statistik nur offen/in_arbeit/erledigt | Py `board_stats` je Spalte | geändert B10 |
| Vollbild (Route `/workspace/{id}/fullscreen`) | UI-Ereignis `fullscreen` | UI geplant / Port (Routing) |
| Lade- und Fehleranzeige, Nur-Lese-Banner „Geteilt von … – Nur Lesezugriff“ | UI | UI geplant |
| Datenbankansicht `useDbKanban`: Gruppieren nach Select-Eigenschaft, führende Spalte „ohne Wert“, Ablegen setzt den Wert | Py `group_by_value`; Core; UI-Variante mit `status_aliases` | Py übernommen, UI geplant |

## 5. Tastatur und Barrierefreiheit

| Funktion (Original) | Ziel | Status |
|---|---|---|
| audit_designer: `N` neue Aufgabe in erster Spalte, `F` Vollbild, `Esc` schließt Detail (nicht in Eingabefeldern; Nur-Lesen nur `Esc`) | UI Tastaturbelegung als Tabelle | UI geplant |
| cockpit: Strg+←/→ zwischen erlaubten Spalten, Strg+↑/↓ innerhalb der Spalte, Ansage über `aria-live` („Karte nach …, Position …“), Enter öffnet | Py `movable_targets`; UI mit ARIA | Py Regel übernommen, UI geplant |
| `prefers-reduced-motion` | UI | UI geplant |
...
| Freigabedialog: Nutzersuche (Name, Benutzername, E-Mail; max. 8; bereits Geteilte ausgeblendet), Rechte ändern, Widerruf mit Bestätigung | UI `ShareDialog`; Nutzerverzeichnis | UI geplant, Port (Nutzerliste) |
| **Befund:** Dialog sendet `"write"`, Backend verlangt `"edit"` → 400 | Import liest `write` als `edit`; UI sendet `edit` | behoben im Ersatz |
| „Mit mir geteilt“ mit Rechte-Label, Eigentümer-Initialen/-Kurzname | Py `list_boards` (Rolle je Board); Core Initialen | Py übernommen, Core/UI |
...
| Anheften, Löschen, relative Zeit („vor 5 Min.“, „vor 2 Tagen“), Fortschrittsbalken | Core `relativeTime`; UI `BoardList` | Core/UI |
...
| Ablegen vor einer Karte, Ablegen nur auf erlaubte Spalten hervorheben | Py `before_id`, `check_move`; UI | Py übernommen, UI geplant |
| Polling alle 10 s mit Sperre während Mutationen, Deduplizierung nach ID | Py Ereignisprotokoll `GET /events?since=`; UI-Store | Py übernommen, UI geplant |
| Auftragsspezifisches (Agent, Profil, Modus, Protokoll, PR, Runner, Kontingent) | `extra` + eigene Detailansicht | Port |

## Offene Punkte

- **Core, UI**: Spiegel in `@flowaudit/kanban-core` und die Vue-/Web-Component-/
  React-Oberfläche entstehen im Branch `feat/kanban-ui`; diese Tabelle wird
