...
ARTIFACT = "auditcore_geo"

#: Catalogue test → (scope, pytest node ids or -k expressions).
CASES: dict[str, tuple[str, list[str]]] = {
    "T-11": (
        "Standorte/Anschriften nur im Speicher; Kontakt und Kennung nicht im Datensatz",
        [
            "tests/test_policy_cases.py::test_t11_locations_are_processed_in_memory_only",
            "tests/test_policy_cases.py::test_t11_geocoder_records_contain_no_contact_or_agent",
        ],
    ),
    "T-14": (
    ...
    "T-38": (
        "Laufzeitmodule nur Standardbibliothek/eigenes Paket; auditcore_harvest nur im Extra",
...
REQUIREMENTS = {
    ...
    "F-15": ("VERSIONED_SOURCE_BOUND_GEO_PROFILES", []),
}
PROFILE_TESTS = [
    "tests/test_contract.py::test_profiles_are_explicit",
    "tests/test_contract.py::test_radius_profiles_differ_measurably",
    "tests/test_contract.py::test_great_circle_matches_each_source_profile",
...
def main() -> int:
    ...
    checks["F-15-profiles"] = {
        "scope": "Erdmodell-Profile mit Kennung und Quellen; Werte gleich dem ausgeführten "
        "Original; Legacy-Replay aller aufgezeichneten Fälle",
        **run_pytest(
            "F-15",
            [
                *PROFILE_TESTS,
    ...
    proof = {
        ...
        "checks": checks,
        "open": {
            "F-05": "Datenschutzbewertung hängt am Consumer (Anschriften beim Geocoding); "
