...
bound to the source digest of ``src/auditcore_registry_sources``, the
...
ARTIFACT = "auditcore_registry_sources"

CASES: dict[str, tuple[str, list[str]]] = {
    "T-09": (
        "Übergabe enthält jede Zeile mit Rohwert, Profil-/Adapterversion und Provenienz; "
        "Zeilenprobleme als Issues; Replay aller Originalausgaben exakt",
        [
            "tests/test_policy_cases.py::test_t09_handover_contains_every_row_with_versions_and_raw_values",
            "tests/test_adapters.py",
            "tests/test_replay_screening.py",
            "tests/test_replay_formats.py",
            "tests/test_replay_sources.py",
            "tests/test_replay_flowinvoice.py",
        ],
    ),
    "T-14": (
        "Bibliothek protokolliert nicht; Ergebnisse und Ereignisse ohne Zugangsdaten",
        ["tests/test_policy_cases.py::test_t14_no_logging_no_print_and_no_secret_in_results"],
    ),
    "T-30": (
        "Fehlende Pflichtspalten, falsche Kodierung und leere Lieferungen werden sichtbar; "
        "Datumsformate folgen dem benannten Modus",
        [
            "tests/test_policy_cases.py::test_t30_renamed_or_missing_columns_are_visible_not_silent",
            "tests/test_formats_contract.py",
        ],
    ),
    "T-38": (
        "Laufzeitmodule: Standardbibliothek, eigenes Paket, auditcore_harvest, "
        "auditcore_entity_matching; rapidfuzz/defusedxml/bs4 nur lazy; "
        "kein DB-/HTTP-/Logging-Zugriff",
...
PROFILE_TESTS = [
    ...
    "tests/test_ownership_and_bulk.py",
]
...
def main() -> int:
    ...
    checks["F-15-profiles"] = {
        ...
        "Profil-ID/-Version; HUMAN_DECISION_REQUIRED in Profil und Ergebnis sichtbar",
    ...
    proof = {
        ...
        "checks": checks,
        "open": {
            "F-05": "Sanktions- und PEP-Listen enthalten Namen natürlicher Personen; Zweck, "
            "Rechtsgrundlage und DSFA-Pflicht sind beim Consumer zu bewerten "
