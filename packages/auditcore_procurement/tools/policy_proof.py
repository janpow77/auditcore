...
ARTIFACT = "auditcore_procurement"

#: Catalogue test → (scope, pytest node ids or -k expressions).
CASES: dict[str, tuple[str, list[str]]] = {
    "T-09": (
        "JSON-Export behält Anzahl, Felder und Datensatzvertrag; Rückimport identisch",
        ["tests/test_policy_cases.py::test_t09_json_export_keeps_count_fields_and_contract"],
    ),
    "T-14": (
        "Bibliothek protokolliert nicht; Harvest-Ergebnisse ohne Header/Zugangsdaten",
        [
            "tests/test_policy_cases.py::"
            "test_t14_library_does_not_log_and_results_contain_no_request_secrets"
        ],
    ),
    "T-30": (
        "Feldaliase/Altfelder, Datumsformate, sichtbare Import-/Parserfehler; "
        "Datensätze tragen Profil- und Adapterversion",
        [
            "tests/test_policy_cases.py::test_t30_field_mapping_variants_errors_and_version_binding",
            "tests/test_legacy_replay.py",
        ],
    ),
    "T-38": (
        "Laufzeitmodule nur Standardbibliothek/eigenes Paket; lxml und auditcore_harvest "
        "nur in den deklarierten Extras",
...
def main() -> int:
    ...
    checks["F-15-profiles"] = {
        "scope": "Precheck-Profil mit Version, Fingerprint und Herkunft; Quellenkatalog "
        "auditcore_harvest.catalog/1 validiert; strict-Ergebnisse nennen das Profil",
        **run_pytest(
            "F-15",
            [
                *PROFILE_TESTS,
    ...
    proof = {
        ...
        "checks": checks,
        "open": {
            "F-05": "Personenbezug möglich (Auftragnehmernamen); Bewertung beim Consumer",
