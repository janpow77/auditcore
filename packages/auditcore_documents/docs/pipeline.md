...
| Aufbewahrung | SQLAlchemy-Service | `RetentionSweeper` mit Port `RetentionStore`, reine Kriterien |
| Profile | – | `LEGACY_PIPELINE` (`flowinvoice.pipeline` 1.0.0), `CORRECTED_PIPELINE` (`auditcore.pipeline` 2026.09.1) |
...
| ID | Befund | Status |
|---|---|---|
| PL-L01 | Englische Zahlenformate werden deutsch normalisiert: `18251.04` → `1825104.0`; „Subtotal: …“ trifft das Muster für `total`; „INVOICE\nDocument:“ ergibt Rechnungsnummer `Document` (Rechnungen aus auditcore_invoicegenerator). | **HUMAN_DECISION_REQUIRED**: Gebietsschema/Feldregeln der Extraktion. |
| PL-L02 | Unlesbare Datumsangaben bleiben als Text; ein Summenwert 0 gilt als „fehlend“ (`all([...])`). | beibehalten |
| PL-L03 | Gateway-Ausfall ergibt Konfidenz 0 und damit REJECTED statt eines technischen Fehlers. | **HUMAN_DECISION_REQUIRED** (Ausfall ≠ unlesbarer Beleg) |
| PL-L04 | Vorverarbeitung ist ein Platzhalter; PIL-Hilfen werden nicht aufgerufen. | beibehalten, Port vorhanden |
| PL-L05 | Nur `processing_logs_days` wird durchgesetzt; Aufbewahrungsfristen für Originale, OCR-Rohdaten, Extraktionen und Audit-Ereignisse sind konfigurierbar, aber wirkungslos. | **HUMAN_DECISION_REQUIRED** (Löschkonzept) |
...
5. Profil: `LEGACY_PIPELINE` hält das bisherige Verhalten; der Wechsel auf
   `CORRECTED_PIPELINE` (PL-C01, PL-C02) ist HUMAN_DECISION_REQUIRED.

`services/extraction_quality_watchdog.py` (C-01…C-13, reine Standardbibliothek,
Consumer `api/fraud_detection.py`) ist **geplant**, nicht Teil dieser Version:
seine Meldungstexte sind ASCII-transliteriert („Nachpruefung“); eine exakte
Übernahme widerspräche der Umlautregel, eine Korrektur ändert Ausgaben
(HUMAN_DECISION_REQUIRED). `services/chandra_ocr.py` bleibt als GPU-Engine in
