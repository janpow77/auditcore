...
ARTIFACT = "auditcore_entity_matching"

#: Catalogue test → (scope, pytest node ids or -k expressions).
CASES: dict[str, tuple[str, list[str]]] = {
    "T-11": (
        "Namen natürlicher Personen werden nur im Speicher verglichen, nicht gespeichert",
        ["tests/test_policy_cases.py::test_t11_personal_names_are_processed_in_memory_only"],
    ),
    "T-14": (
    ...
    "T-38": (
        "Laufzeitmodule nutzen nur Standardbibliothek/eigenes Paket, rapidfuzz nur lazy",
...
REQUIREMENTS = {
    ...
    "F-15": ("VERSIONED_SOURCE_BOUND_MATCHING_PROFILES", []),
}
PROFILE_TESTS = [
    "tests/test_contract.py::test_profiles_are_explicit_source_bound_and_fingerprinted",
    "tests/test_contract.py::test_profile_constants_equal_the_executed_source",
    "tests/test_contract.py::test_profile_validation_and_lookup_errors",
]
...
def main() -> int:
    ...
    checks["F-15-profiles"] = {
        "scope": "Vier quellengebundene Profile mit Version, Fingerprint, Herkunft; Werte "
        "gleich dem ausgeführten Original; Ergebnisse nennen die Profilidentität",
        **run_pytest(
            "F-15",
            [
                *PROFILE_TESTS,
    ...
    proof = {
        ...
        "checks": checks,
        "open": {
            "F-05": "Datenschutzbewertung hängt am Consumer (Screening natürlicher Personen); "
