...
ARTIFACT = "auditcore_harvest"

#: Catalogue test → (scope, pytest targets).
CASES: dict[str, tuple[str, list[str]]] = {
    "T-05": (
        "Laufzustände: Abbruch, Limits, Wiederanlauf am bestätigten Cursor, fremde Profilversion "
        "abgewiesen, Historie (Checkpoint vorher/nachher) erhalten",
        [
            "tests/test_engine.py::test_cancel_and_config_and_source_mismatch",
            "tests/test_engine.py::test_limits_end_the_run_as_partial",
            "tests/test_engine.py::test_sink_failure_keeps_checkpoint_and_resume_loses_nothing",
            "tests/test_engine.py::test_profile_version_change_refuses_foreign_cursor",
            "tests/test_engine.py::test_finished_snapshot_restarts_but_incremental_continues",
        ],
    ),
    "T-09": (
        "Übergabe/Export enthält genau die geernteten Datensätze mit Zahl, Provenienz und "
        "Profilversion",
        [
            "tests/test_cli.py::test_replay_exports_exactly_the_harvested_records",
            "tests/test_engine.py::test_pages_are_followed_until_complete_and_checkpointed",
            "tests/test_engine.py::test_duplicates_within_run_and_at_sink_are_counted",
        ],
    ),
    "T-14": (
        "Ereignisse begrenzt, ohne Konfiguration und Zugangsdaten; Kern ohne eigenes Logging",
        [
            "tests/test_engine.py::test_events_are_bounded_and_carry_no_config",
            "tests/test_reference_contract.py",
        ],
    ),
    "T-30": (
        "Quellprofilversion in Provenienz und Checkpoint; Profilwechsel ohne fremden Cursor; "
        "fehlende Felder als Hinweis statt stiller Übernahme; Replay als Vorschau",
        [
            "tests/test_engine.py::test_profile_version_change_refuses_foreign_cursor",
            "tests/test_engine.py::test_issues_make_the_run_partial_not_complete",
            "tests/test_transport_reference.py::test_json_adapter_filters_issues_and_secret_handling",
            "tests/test_cli.py::test_replay_exports_exactly_the_harvested_records",
        ],
    ),
    "T-38": (
        "Laufzeitmodule nur Standardbibliothek, kein Netzwerkclient, keine DB/ORM/Mandanten",
...
REQUIREMENTS = {
    "F-03": ("RUN_STATE_AND_RESUME", ["T-05"]),
    "F-04": ("RECORD_HANDOVER_AND_REPLAY_EXPORT", ["T-09"]),
    "F-07": ("LIBRARY_EVENTS_AND_SUPPLY_CHAIN", ["T-14", "T-37"]),
    "F-09": ("LIBRARY_SOURCE_ARCHITECTURE", ["T-38"]),
    "F-16": ("VERSIONED_SOURCE_PROFILES", ["T-30"]),
}
...
def main() -> int:
    ...
    proof = {
        ...
        "checks": checks,
        "open": {
            "F-05": "Geerntete Inhalte können personenbezogen sein; VVT/DSFA des "
            "Datenflusses beim Consumer; dsfa_required UNKNOWN",
            "F-07.ASSESS": "Schutzbedarf der Bibliothek nicht festgelegt (protection_need UNKNOWN)",
            "T-12": "Anwendbarkeit UNKNOWN (dsfa_required); DSFA-Bewertung ist Consumeraufgabe",
        },
    }
