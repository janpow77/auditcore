...
bound to the source digest of ``src/auditcore_price_sources``, the
...
ARTIFACT = "auditcore_price_sources"

CASES: dict[str, tuple[str, list[str]]] = {
    "T-09": (
        "Übergabe enthält jeden Datensatz mit Adapter-/Profilversion, Rohwert-Hash, Einheit und "
        "Zeitbezug; Vergleich mit dem ausgeführten Original (Anfragen, gespeicherte Zeilen, "
        "Hashes)",
        [
            "tests/test_policy_cases.py::test_t09_handover_has_every_record_with_unit_time_reference_and_versions",
            "tests/test_characterization.py",
            "tests/test_contract.py",
        ],
    ),
    "T-14": (
        "Keine Zugangsdaten in Ergebnis, Ereignissen, Datensätzen und aufgezeichneten Anfragen; "
        "Bibliothek protokolliert nicht",
        [
            "tests/test_policy_cases.py::test_t14_results_events_and_snapshots_contain_no_secrets",
            "tests/test_policy_cases.py::test_t14_library_does_not_log_or_print",
        ],
    ),
    "T-30": (
        "Einheitenherkunft benannt, fehlende Werte bleiben None, unlesbare Zeilen als Hinweis; "
        "Parser-Grenzfälle",
        [
            "tests/test_policy_cases.py::test_t30_source_unit_and_missing_values_are_never_silently_replaced",
            "tests/test_parsers.py",
        ],
    ),
    "T-38": (
        "Laufzeitmodule: Standardbibliothek, eigenes Paket und auditcore_harvest; kein "
        "Netzwerkclient, keine DB, kein Logging",
...
PROFILE_TESTS = ["tests/test_catalog.py", "tests/test_contract.py"]
...
def main() -> int:
    ...
    checks["F-15-profiles"] = {
        "scope": "Versionierter Quellenkatalog; jede Quelle mit Adapter-/Profilversion, "
        "Herkunft, Einheit, Zeitbezug und Datenrechtestatus REVIEW_REQUIRED",
    ...
    proof = {
        ...
        "checks": checks,
        "open": {
            "F-05": "Personenbezug möglich (Stationsnamen/Betreiber); beim Consumer zu bewerten; "
