...
SOURCES: dict[str, dict[str, Any]] = {
    ...
    "flowsearch": {
        ...
        "root": "backend",
        "blobs": {
            "app/services/eu_beneficiary_harvester_v2.py": "4cbc58a2057e896808a079808d8f6fa968c2f79f",
    ...
    "designer": {
        ...
        "root": "backend",
        "blobs": {
            "app/core/shared/research/register/beneficiaries.py": "88f8d649c6a43607daaabab93a16cb6fdf82a8b2",
            "app/core/shared/research/register/state_aid.py": "79fb0ab5aa2082a3c080867bec9e8e14d3245b06",
            "app/core/shared/research/register/de_minimis.py": "e0a660aa902933d5c7f6a550b908e8b0b1b0be16",
            "app/core/shared/research/register/de_minimis_ernte.py": "9ad78b1bf491d546caea4e161d8a140d8853e161",
            "app/core/shared/research/register/de_minimis_models.py": "b73451505740935ff201968295ce5b2e2c12b582",
            "app/core/shared/research/register/de_minimis_zuordnung.py": "56e1f5053ba1aa356be64a505b6c62d8a3489d52",
        },
        "database_env": {"DATABASE_URL": "postgresql://nobody:nobody@127.0.0.1:1/never"},
    },
}
...
AMOUNTS: list[Any] = [
    None, "", " ", "-", "—", "–", "0", "1234", "1.234", "1.234.567", "1.234,56", "1,234.56",
    "1,234,567", "1,5", "1,50", "1,500", "12,3456", "1.2.3", "1.000.000,00 €", "€ 1.200",
    "EUR 1.200,00", "1.200,00 EUR", "usd 10", "150.000,00 €", "500,001 to 1,000,000",
    "500.001 bis 1.000.000", "less than 100,000", "weniger als 100.000", "< 50000",
    "more than 30,000,000", "mehr als 5", "> 10", "1\xa0234,56", "1 234 567", "-1.500,00",
    "abc", "12e3", "0,5", "1.5", "100.", ".5", "1,2,3", 1234, 1234.5, 0, -3,
    float("nan"), Decimal("12.50"), True,
]
DATES: list[Any] = [
    None, "", "2024-03-15", "15.03.2024", "15/03/2024", "2024/03/15", "15.03.24", "2024-03-15T00:00:00",
    "2024-03-15 10:00", "31.02.2024", "3/4/2024", " 2024-01-01 ", "01.01.1900", "20240315",
    datetime(2024, 3, 15, 12, 30), date(2024, 3, 15), 45000, "Q1 2024",
]
NAMES: list[Any] = [
    None, "", "Beispiel GmbH", "  BEISPIEL   gmbh  ", "Müller & Söhne KG", "Fraunhofer-Gesellschaft e.V.",
    "ÄÖÜ Straße ß GmbH & Co. KG", "Holding Group Deutschland AG", "ACME Ltd.", "Société Générale S.A.",
    "Łódź Sp. z o.o.", "Dvořák s.r.o.", "gGmbH Stiftung", "Co. KG", "Gruppe International", "123 GmbH",
    "Her************", "o’Brien & Co", "Peter\tMeier\nGmbH",
]
SA_TEXTS: list[Any] = [
    None, "", "SA.12345", "sa 12345", "SA-12345/2021", "SA.123456", "SA.1234", "SA.123", "Beihilfe SA_54321.2020",
    "XSA.12345", "SA12345/20", "Verweis auf SA.98765 und SA.11111", "N 123/2010",
]
...
def capture_flowworkshop(root: Path, fixtures: Path, rec: Recorder) -> dict[str, Any]:
    ...
    for i, v in enumerate(AMOUNTS):
        rec.observe(f"parse_amount-{i}", "workshop.parse_amount", {"value": v}, lambda v=v: sa.parse_amount(v))
    for i, v in enumerate(DATES):
        rec.observe(f"parse_date-{i}", "workshop.parse_date", {"value": v}, lambda v=v: sa.parse_date(v))
    for i, v in enumerate(NAMES):
        ...
        rec.observe(f"strip_accents-{i}", "workshop.strip_accents", {"value": v},
                    lambda v=v: sa._strip_accents(v))
        rec.observe(f"normalize_for_hash-{i}", "workshop.normalize_for_hash", {"value": v},
                    lambda v=v: bh._normalize_for_hash(v))
        rec.observe(f"normalize_name_simple-{i}", "workshop.normalize_company_name_simple",
                    {"value": v}, lambda v=v: bh._normalize_company_name_simple(v))
    for i, v in enumerate(SA_TEXTS):
        rec.observe(f"detect_sa-{i}", "workshop.detect_sa_reference", {"value": v},
                    lambda v=v: sa.detect_sa_reference(v))
    for i, v in enumerate([None, float("nan"), "", " x ", 12345.0, 1234.0, 12345.5, "01067", 1067, 0, "  "]):
        rec.observe(f"stringify-{i}", "workshop.stringify", {"value": v}, lambda v=v: bh._stringify(v))
        rec.observe(f"stringify_plz-{i}", "workshop.stringify_plz", {"value": v},
                    lambda v=v: bh._stringify_plz(v))
    for i, v in enumerate([None, "", "50,1", "50.1", " 8,25 ", "x", 7, 7.5, float("nan"), True]):
        rec.observe(f"coerce_float-{i}", "workshop.coerce_float", {"value": v},
    ...
    hash_rows = {
        ...
        "without-context": {k: v for k, v in base_row.items() if k not in ("bundesland", "periode", "fonds")},
    ...
    for name, row in hash_rows.items():
        for key in ("hessen_efre_2021_2027", "src1", ""):
            rec.observe(f"hash-{name}-{key}", "workshop.compute_record_hash", {"row": row, "source_key": key},
                        lambda r=row, k=key: bh.compute_record_hash(r, k))
    headers_sets = {
        "german": ["Name des Begünstigten", "Bezeichnung des Vorhabens", "Gesamtkosten", "Unionsbeteiligung",
                   "PLZ", "Ort", "Datum des Beginns", "Datum des Endes", "Aktenzeichen"],
        "english": ["beneficiary_name", "operation_name", "total_eligible_cost", "eu_funding", "postcode",
                    "city", "start_date", "end_date", "operation_id"],
        "ambiguous": ["Name", "Firmenname", "Projekt", "Projektkosten", "Betrag", "Standort PLZ", "Standort Ort"],
        "empty": [],
        "no-name": ["Projekt", "Kosten"],
    }
    mappings: list[Any] = [None, {}, {"name": "Firmenname"}, {"name": "fehlt"}, {"unbekannt": "Projekt"},
                           {"kosten": "Betrag", "plz": "Standort PLZ"}]
    for hname, headers in headers_sets.items():
        for mi, mapping in enumerate(mappings):
            rec.observe(f"columns-{hname}-{mi}", "workshop.detect_canonical_columns",
        ...
        rec.observe(
            ...
            lambda c=content, n=file_name, s=spec: list(
                bh.parse_xlsx_or_csv(
                    c, file_name=n, header_row=s.get("header_row", 0),
                    field_mapping=s.get("field_mapping"), sheet=s.get("sheet"),
                )
            ),
        )
        if "field_mapping" not in spec:
            rec.observe(f"read-{file_name}", "workshop.read_table", {"file": file_name, **spec},
                        lambda c=content, n=file_name: _frame(dfs, c, n))

    rows_named = [{"_row_number": i + 1, "beneficiary_name": f"N{i}", "raw_row": {"Fonds": f}}
                  for i, f in enumerate(["EFRE", "JTF", "EFRE", "ESF+", "efre "])]
    for fonds in (None, "", "EFRE", "JTF", "ESF", "ESF+", "KOHÄSION"):
        rec.observe(f"fund-filter-{fonds}", "workshop.filter_by_fund", {"rows": rows_named, "fonds": fonds},
                    lambda f=fonds: bh.filtere_nach_fonds(copy.deepcopy(rows_named), f))
    single = [{"_row_number": 1, "raw_row": {"Fonds": "EFRE"}}, {"_row_number": 2, "raw_row": {"Fonds": "EFRE"}}]
    rec.observe("fund-filter-uniform", "workshop.filter_by_fund", {"rows": single, "fonds": "JTF"},
                lambda: bh.filtere_nach_fonds(copy.deepcopy(single), "JTF"))
    headerish = [{"_row_number": 1, "raw_row": {"Fund concerned": "Fund concerned"}},
                 {"_row_number": 2, "raw_row": {"Fund concerned": "ERDF"}}]
    rec.observe("fund-filter-header-remnant", "workshop.filter_by_fund", {"rows": headerish, "fonds": "ERDF"},
                lambda: bh.filtere_nach_fonds(copy.deepcopy(headerish), "ERDF"))

    def rows(total: int, nameless: int, **extra: Any) -> list[dict[str, Any]]:
        out = [{"_row_number": i + 1, "beneficiary_name": f"B{i}", **extra} for i in range(total - nameless)]
        out += [{"_row_number": total - nameless + i + 1, "_skip_reason": "no_name"} for i in range(nameless)]
        return out
    ...
    validation_cases = {
        ...
        "negative": ([{"_row_number": 1, "cost_total_raw": "-5", "cost_eu_funding_raw": "-1"}], params_ok),
        "eu-over-total": ([{"_row_number": 1, "cost_total_raw": "100", "cost_eu_funding_raw": "150"}], params_ok),
        "dates": ([{"_row_number": 1, "project_start_raw": "2025-01-01", "project_end_raw": "2024-01-01"}],
                  params_ok),
        "coords": ([{"_row_number": 1, "latitude": 91.0, "longitude": -181.0}], params_ok),
        "unparsable": ([{"_row_number": 1, "cost_total_raw": "abc", "project_start_raw": "x"}], params_ok),
    }
    for name, (rws, prm) in validation_cases.items():
        rec.observe(f"validate-{name}", "workshop.validate_rows", {"rows": rws, "params": prm},
    ...
    return {
        ...
        "nameless": {"max_share": bh.NAMENLOS_MAX_ANTEIL, "tolerance": bh.NAMENLOS_TOLERANZ,
        ...
        "transliteration": {chr(k): v for k, v in str.maketrans({
            "ä": "ae", "ö": "oe", "ü": "ue", "ß": "ss", "Ä": "ae", "Ö": "oe", "Ü": "ue"}).items()},
        "filler_words": sorted(sa._FILLER_WORDS),
        "files": {n: {"sha256": hashlib.sha256((files / n).read_bytes()).hexdigest(), **s}
...
def capture_workshop_modes(database_url: str, files: Path) -> list[dict[str, Any]]:
    ...
    changed = base.replace(b"150000;75000", b"160000;80000").replace(b"Ohne Kosten e.V.;Beratung;;;",
                                                                     b"Neu GmbH;Beratung;;;")
    duplicate = base + "Beispiel GmbH;Energie 2030;1.234.567,89;617.283,95;01067;Dresden;01.02.2024;".encode() \
        + "31.12.2026;AZ-1;EFRE\n".encode()
    rejected = b"Name des Begr\xc3\xbcnstigten;Gesamtkosten\nA;-5\n"
    context = {"source_key": "hessen_efre", "bundesland": "Hessen", "fonds": "EFRE",
               "periode": "2021-2027", "country_code": "DE", "file_name": "liste.csv"}
    steps = [
    ...
    with Session(engine) as session:
        for name, mode, content in steps:
            ...
            if name == "no-context":
                params = {"source_key": "hessen_efre", "mode": mode, "file_content": content,
                          "file_name": "liste.csv"}
            try:
            ...
            records = session.query(BeneficiaryRecord).order_by(BeneficiaryRecord.source_record_id).all()
            out.append({
                "name": name, "mode": mode, "result": jsonable(result), "exception": exception,
                "inventory": [[r.source_record_id, r.beneficiary_name, r.cost_total_raw,
                               str(r.cost_total) if r.cost_total is not None else None, r.plz]
                              for r in records],
            })
...
def _workshop_files(files: Path, openpyxl: Any) -> dict[str, dict[str, Any]]:
    ...
    write(
        ...
        (
            ...
            "Stand: 01.09.2026\n"
            "\n"
            "Name des Begünstigten;Bezeichnung des Vorhabens;Gesamtkosten;Unionsbeteiligung;PLZ;Ort;"
            "Datum des Beginns;Datum des Endes;Aktenzeichen;Fonds\n"
            "Beispiel GmbH;Energie 2030;1.234.567,89;617.283,95;01067;Dresden;01.02.2024;31.12.2026;AZ-1;EFRE\n"
            "Müller & Söhne KG;Digitalisierung;150000;75000;34117;Kassel;2024-03-15;2025-03-15;2;EFRE\n"
            ";Summe;1.384.567,89;;;;;;;\n"
    ...
    write("ohne_namensspalte.csv", "Projekt;Kosten;Ort\nP1;10;X\nP2;20;Y\n".encode())
    ...
    if not (files / "titelzeilen.xlsx").exists():
        ...
        ws.append([])
        ws.append(["Name des Begünstigten", "Bezeichnung des Vorhabens", "Gesamtkosten", "PLZ", "Ort",
                   "Datum des Beginns", "Breitengrad", "Längengrad"])
        ws.append(["Beispiel GmbH", "Qualifizierung", 125000.5, 1067, "Dresden", datetime(2024, 2, 1), 51.05, 13.74])
        ws.append(["Zweite AG", "Beratung", 80000, 34117, "Kassel", datetime(2024, 3, 15), None, None])
        ws.append([None, "Summe", 205000.5, None, None, None, None, None])
        ws.append(["Dritte eG", 12345, "1.000,00", "60311", "Frankfurt", "15.03.2024", "50,1", "8,7"])
        buffer = io.BytesIO()
...
def capture_flowsearch(root: Path, fixtures: Path, rec: Recorder) -> dict[str, Any]:
    ...
    for i, v in enumerate(AMOUNTS):
        rec.observe(f"parse_amount-{i}", "flowsearch.parse_amount", {"value": v},
                    lambda v=v: harvester._parse_amount(v))
    for i, v in enumerate(DATES):
        rec.observe(f"parse_date-{i}", "flowsearch.parse_date", {"value": v}, lambda v=v: harvester._parse_date(v))
    for i, v in enumerate(NAMES):
        if isinstance(v, str):
            rec.observe(f"normalize_name-{i}", "flowsearch.normalize_name", {"value": v},
                        lambda v=v: harvester._normalize_name(v))
    for i, v in enumerate([None, "", "70173 Stuttgart", "Stuttgart, 70173", "70173", "Stuttgart", "1234 Wien",
                           " 01067  Dresden ", "Berlin,12345", "D-12345 Berlin"]):
        rec.observe(f"parse_location-{i}", "flowsearch.parse_location", {"value": v},
                    lambda v=v: harvester._parse_location(v))
    for i, (v, n) in enumerate([(None, 5), ("abc", 5), ("abcdef", 5), ("äöüß" * 3, 5)]):
        rec.observe(f"truncate-{i}", "flowsearch.truncate", {"value": v, "max": n},
    ...
    for field_name in ("beneficiary_name", "total_cost", "location", "project_name"):
        rec.observe(f"extract-{field_name}", "flowsearch.extract_mapped_field",
    ...
    if not (files / xlsx_name).exists():
        ...
        ws = wb.active
        ws.append(["Titel"])
        ws.append(["Begünstigter", "Vorhaben\nName", "Gesamtkosten", "EU-Beitrag", "Ort", "Beginn", None])
        ws.append(["Beispiel GmbH", "Energie", 1234.56, 600, "70173 Stuttgart", datetime(2024, 2, 1), "x"])
    ...
    rec.observe("parse_csv", "flowsearch.parse_csv", {"file": csv_name, "mapping": mapping},
    ...
    rec.observe("parse_excel", "flowsearch.parse_excel", {"file": xlsx_name, "mapping": xlsx_mapping},
                lambda: asyncio.run(harvester._parse_excel(xlsx_bytes, source)))
    rec.observe("extract_zip", "flowsearch.extract_from_zip", {"file": zip_name},
                lambda: harvester._extract_from_zip((files / zip_name).read_bytes()))
    rec.observe("extract_zip_empty", "flowsearch.extract_from_zip", {"file": "(no table)"},
    ...
    for i, rec_row in enumerate([*records, {"Begünstigter": "  "}, {"Unbekannt": "x"}]):
        def imported(row: dict[str, Any] = rec_row) -> Any:
            ...
            if result is None:
                return None
            fields = ("project_id", "beneficiary_name", "beneficiary_name_normalized", "fund_type", "nuts0",
                      "nuts1", "eu_cofinancing", "national_cofinancing", "total_amount",
                      "total_eligible_expenditure", "project_title", "project_summary", "start_date",
                      "end_date", "city", "postal_code", "data_source", "data_source_url",
                      "programming_period")
            return {f: getattr(result, f) for f in fields}

        rec.observe(f"import_record-{i}", "flowsearch.import_record", {"record": rec_row, "source": source},
                    imported)
    return {
...
def capture_designer(root: Path, fixtures: Path, rec: Recorder) -> dict[str, Any]:
    ...
    for i, v in enumerate(AMOUNTS):
        rec.observe(f"parse_betrag-{i}", "designer.parse_betrag", {"value": v}, lambda v=v: bn.parse_betrag(v))
        rec.observe(f"sa_parse_amount-{i}", "designer.state_aid_parse_amount", {"value": v},
                    lambda v=v: sa.parse_amount(v))
        rec.observe(f"betrag_ist_spanne-{i}", "designer.amount_is_range", {"value": v},
                    lambda v=v: sa.betrag_ist_spanne(v))
    for i, v in enumerate([None, "", "42 %", "29,07\xa0%", "50.00 %", 60, 44.0978, 0.49204867971092, "0,5", "1",
                           "1 %", "150", "4400", "-5", "85 Prozent", "x"]):
        rec.observe(f"parse_satz-{i}", "designer.parse_satz", {"value": v}, lambda v=v: bn.parse_satz(v))
    for i, v in enumerate(DATES):
        rec.observe(f"parse_datum-{i}", "designer.parse_datum", {"value": v}, lambda v=v: bn.parse_datum(v))
        rec.observe(f"sa_parse_date-{i}", "designer.state_aid_parse_date", {"value": v},
                    lambda v=v: sa.parse_date(v))
    for i, v in enumerate(NAMES):
        for filler in (False, True):
            rec.observe(f"sa_normalize-{i}-{filler}", "designer.normalize_company_name",
                        {"value": v, "drop_filler": filler},
                        lambda v=v, f=filler: sa.normalize_company_name(v, drop_filler=f))
    for i, v in enumerate(SA_TEXTS):
        rec.observe(f"sa_detect-{i}", "designer.detect_sa_reference", {"value": v},
                    lambda v=v: sa.detect_sa_reference(v))
    base_row = {
        "beneficiary_name": "Beispiel GmbH", "project_name": "Energieeffizienz 2030",
        "project_aktenzeichen": "AZ-12345", "bundesland": "Hessen", "periode": "2021-2027",
        "fonds": "EFRE", "funded_at_raw": "2024-03-15", "cost_total_raw": "150.000,00 €",
    }
    for name, row in {"full": base_row, "whitespace": {**base_row, "beneficiary_name": "  Beispiel   GmbH "},
                      "without-context": {k: v for k, v in base_row.items()
                                          if k not in ("bundesland", "periode", "fonds")},
                      "nan": {**base_row, "cost_total_raw": float("nan")}, "empty": {}}.items():
        rec.observe(f"hash-{name}", "designer.compute_record_hash", {"row": row, "source_key": "src1"},
                    lambda r=row: bn.compute_record_hash(r, "src1"))

    # --- de-minimis: pure parts
    for i, v in enumerate([None, "", "DE", "de", "DEU", "CountryDEU", "countrydeu", "AT", "EL", "GR", "XX", "USA"]):
        rec.observe(f"landeskennung-{i}", "deminimis.country_code", {"value": v},
                    lambda v=v: dm.landeskennung(v))
    for i, kw in enumerate([{}, {"beneficiaryName": "Beispiel", "deMinimisTypes": [], "amountFrom": 0.0},
                            {"country": None, "referenceNumber": "", "pageNumber": 3, "pageSize": 500}]):
        rec.observe(f"criteria-{i}", "deminimis.criteria_body", kw,
                    lambda kw=kw: dm.Suchkriterien(**kw).als_koerper())
    for i, v in enumerate([None, "", "2024-03-15", "2024-03-15T10:00:00Z", "15.03.2024", date(2024, 1, 1), "x"]):
        rec.observe(f"as_date-{i}", "deminimis.as_date", {"value": v}, lambda v=v: dm._als_datum(v))
        rec.observe(f"harvest_date-{i}", "deminimis.harvest_date", {"value": v}, lambda v=v: de._datum(v))
    for i, v in enumerate([None, "", "1000", "1000.50", 1000, 1000.5, "1.000,50", "NaN", "Infinity", True, "x"]):
        rec.observe(f"as_amount-{i}", "deminimis.as_amount", {"value": v}, lambda v=v: dm._als_betrag(v))
        rec.observe(f"harvest_amount-{i}", "deminimis.harvest_amount", {"value": v}, lambda v=v: de._betrag(v))
    for i, v in enumerate([None, "", "2026-01-15 10:00:00", "2026-01-15T10:00:00", "x"]):
        rec.observe(f"harvest_timestamp-{i}", "deminimis.harvest_timestamp", {"value": v},
                    lambda v=v: de._zeitpunkt(v))
    for i, v in enumerate([None, "", "  Beispiel   GmbH ", "Her************"]):
        rec.observe(f"harvest_normalize-{i}", "deminimis.harvest_normalize", {"value": v},
    ...
    for i, v in enumerate([None, "", "DE-RP-HWK Rheinhessen", "Hessisches Ministerium für Wirtschaft",
                           "Bundesamt für Wirtschaft und Ausfuhrkontrolle (BAFA)", "KfW Bankengruppe",
                           "Grenke Bank AG", "Landratsamt Görlitz", "Stadt Leipzig", "Sachsen-Anhalt Invest",
                           "IB.SH Kiel", "Regierungspräsidium Kassel", "NRW.BANK", "bafa", "Rheinhessen"]):
        rec.observe(f"authority_level-{i}", "deminimis.authority_level", {"value": v},
                    lambda v=v: dz.ebene(v))
    rec.observe("row", "deminimis.row", {"award": award}, lambda: dm._zeile(award))
    masked = {**award, "beneficiaryName": "Her************", "grantingAuthorityName": "Bundesamt für X"}
    for i, a in enumerate([award, masked, {}, {"country": None, "grantingAuthorityName": "Landratsamt Y"}]):
        rec.observe(f"fields-{i}", "deminimis.harvest_fields", {"award": a}, lambda a=a: de._felder(a))
        rec.observe(f"satz_hash-{i}", "deminimis.record_hash", {"award": a}, lambda a=a: dmm.satz_hash(a))
    rec.observe("bestand_hash", "deminimis.inventory_hash", {"hashes": ["b", "a", "c"]},
                lambda: dmm.bestand_hash(["b", "a", "c"]))
    rec.observe("bestand_hash-empty", "deminimis.inventory_hash", {"hashes": []}, lambda: dmm.bestand_hash([]))
    ...
    cumulation_cases = {
        ...
        "string-amounts": ([aw("2025-01-01", "1000.10"), aw("2025-01-02", "0.20")], date(2026, 9, 1)),
    ...
    for name, (records, day) in cumulation_cases.items():
        rec.observe(f"cumulation-{name}", "deminimis.cumulation", {"records": records, "stichtag": day},
                    lambda r=records, d=day: dm.berechne_kumulierung(r, stichtag=d).to_dict())
        rec.observe(f"cumulation-raw-{name}", "deminimis.cumulation_raw", {"records": records, "stichtag": day},
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if path.endswith("/de-minimis-aid-awards") and body and body.get("beneficiaryName") == "fehler":
            return httpx.Response(500, json={"error": "x"})
        if path.endswith("/de-minimis-aid-awards") and body and body.get("beneficiaryName") == "kaputt":
            return httpx.Response(200, json={"unerwartet": True})
        if path.endswith("/de-minimis-aid-awards"):
            page = body.get("pageNumber", 0)
            return httpx.Response(200, json=[{**award, "referenceNumber": f"DM-{page}-{i}"} for i in range(2)])
    ...
    for i, kw in enumerate([{}, {"beneficiaryName": "fehler"}, {"beneficiaryName": "kaputt"},
                            {"pageNumber": 2, "pageSize": 2}]):
        rec.observe(f"client-search-{i}", "deminimis.client_search", kw,
                    lambda kw=kw: client().suche(dm.Suchkriterien(**kw)))
    for i, kw in enumerate([{}, {"beneficiaryName": "liste"}, {"beneficiaryName": "ohne"}]):
        rec.observe(f"client-count-{i}", "deminimis.client_count", kw,
                    lambda kw=kw: client().anzahl(dm.Suchkriterien(**kw)))
    for ref in ("B-000123456", "B-404", "B-503"):
        rec.observe(f"client-beneficiary-{ref}", "deminimis.client_beneficiary", {"reference": ref},
                    lambda r=ref: client().je_beguenstigtem(r))
    rec.observe("client-network", "deminimis.client_search", {"network": "down"},
    ...
    def pages(pages_data: list[list[dict[str, Any]]], reported: int | None, fail_at: int | None = None) -> Any:
    ...
    with Session(engine) as session:
        for name, fetch in scenarios:
            ...
            awards = session.query(dmm.ResearchDeMinimisAward).order_by(
                dmm.ResearchDeMinimisAward.reference_number).all()
            harvest_steps.append(
                {
                    "name": name,
                    "run": {k: jsonable(getattr(run, k)) for k in (
                        "status", "records_reported", "records_seen", "records_inserted", "records_updated",
                        "records_unchanged", "records_vanished", "requests", "content_hash", "error_message",
                        "country", "source_url")},
                    "inventory": {k: jsonable(v) for k, v in state.items()
                                  if k not in ("stand_am", "letzter_lauf")},
                    "award_count": len(awards),
                    "vanished_count": sum(a.verschwunden_seit is not None for a in awards),
                    "awards_digest": hashlib.sha256(json.dumps(
                        [[a.reference_number, str(a.amount_eur), a.verschwunden_seit is not None,
                          a.aenderungen, a.content_hash] for a in awards]).encode()).hexdigest(),
                    "awards": [
                        {"reference": a.reference_number, "amount": jsonable(a.amount_eur),
                         "vanished": a.verschwunden_seit is not None, "changes": a.aenderungen,
                         "hash": a.content_hash}
                        for a in awards
                        if not a.reference_number.startswith("A-") or int(a.reference_number[2:]) < 5
                    ],
                }
            )
    return {
        ...
        "notices": {"company": dm.VORBEHALT_UNTERNEHMEN, "period": dm.VORBEHALT_ZEITRAUM,
                    "deadline": dm.VORBEHALT_FRIST, "name": dm.VORBEHALT_NAME},
...
def main() -> None:
    ...
    for relative, expected in spec["blobs"].items():
        ...
        files.append({"path": f"{spec['root']}/{relative}", "git_blob": observed,
                      "pinned_blob": expected})
    ...
    rec = Recorder()
    capture = {"flowworkshop": capture_flowworkshop, "flowsearch": capture_flowsearch,
               "designer": capture_designer}[args.variant]
    constants = capture(root, args.fixtures, rec)
    if args.modes_database:
        from urllib.parse import urlparse

        target = urlparse(args.modes_database)
        if args.variant != "flowworkshop" or target.hostname != "127.0.0.1" or "capture" not in (
            target.path or ""
        ):
            raise SystemExit("Refusing: modes database must be a disposable local capture database")
        constants["modes_run"] = capture_workshop_modes(args.modes_database, args.fixtures / "files")
    library = None
    if args.migrated:
    ...
    report = {
        ...
        "source": {"repository": spec["repository"], "commit": spec["commit"], "head": head,
                   "files": files, "checkout_clean": not dirty, "migrated": args.migrated,
                   "library": library},
    ...
    print(json.dumps({"variant": args.variant, "cases": len(rec.cases),
