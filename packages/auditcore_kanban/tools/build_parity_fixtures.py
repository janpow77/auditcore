...
def card(cid: str, column: str, rank: str, **fields: Any) -> Card:
    return Card(cid, column, rank, fields.pop("title", cid.upper()), created_at="2026-09-01",
                **fields)
...
def base_board(**changes: Any) -> Board:
    ...
    cards = (
        card("a", "offen", "V", title="Prüfbericht Entwurf", tags=("VP", "Bericht"),
             priority="hoch", due="2026-09-20", badge="VP-19", assignees=("u2",)),
        card("b", "offen", "k", title="Belegliste anfordern", description="Straße prüfen",
             due="2026-09-27"),
        card("c", "in_arbeit", "V", title="Vergabe prüfen", priority="niedrig", tags=("Vergabe",),
             due="2026-10-30"),
...
def rank_cases() -> list[Case]:
    pairs = [(None, None), (None, "V"), ("V", None), ("A", "B"), ("A", "C"), ("V", "V1"),
             ("0V", "1"), ("z", None), ("zz", None), (None, "01"), (None, "001"), ("a", "a1"),
             ("Az", "B"), ("y1", "z"), ("V", "V"), ("W", "V"), ("V0", None), ("", "V"),
             ("A!", None), ("V", "W"), ("V", "V01")]
    cases = [case(f"between {a!r} {b!r}", {"op": "between", "a": a, "b": b}, _rank_result(a, b))
             for a, b in pairs]
    cases += [case(f"spread {n}", {"op": "spread", "count": n}, spread_ranks(n))
              for n in (0, 1, 2, 3, 5, 10, 61, 62, 100)]
    cases += [case(f"valid {k!r}", {"op": "valid", "key": k}, is_valid_rank(k))
...
def _policy_boards() -> dict[str, Board]:
    restricted = TransitionPolicy(allowed=frozenset({("offen", "in_arbeit"),
                                                     ("in_arbeit", "erledigt")}))
    return {
        "free": base_board(),
...
def transition_cases() -> list[Case]:
    moves = [("a", "offen"), ("a", "in_arbeit"), ("a", "erledigt"), ("c", "offen"),
             ("c", "erledigt"), ("d", "offen"), ("b", "gibt_es_nicht")]
    cases = []
    for name, board in _policy_boards().items():
        ...
        for label, variant in ((name, board), (f"{name}+full", full)):
            for card_id, target in moves:
                ...
                cases.append(case(f"{label}: {card_id}->{target}",
...
def wip_cases() -> list[Case]:
    ...
    for name, board in (("block", base_board()), ("warn", base_board(wip_mode="warn"))):
        full = _replace(board, "cards", board.cards + (card("e", "in_arbeit", "k"),))
        for label, variant in ((name, board), (f"{name}+full", full)):
            for column, exclude in (("in_arbeit", None), ("in_arbeit", "c"), ("offen", None),
                                    ("nope", None)):
                cases.append(case(f"{label}: {column} without {exclude}",
                                  {"board": bj(variant), "column_id": column,
                                   "exclude_card_id": exclude},
...
def filter_cases() -> list[Case]:
    ...
    filters: list[tuple[str, dict[str, Any]]] = [
        ("empty", {}), ("query title", {"query": "prüf"}), ("query case", {"query": "VERGABE"}),
        ("query description umlaut", {"query": "STRASSE"}), ("query ß", {"query": "straße"}),
        ("query tag", {"query": "bericht"}), ("query badge", {"query": "vp-19"}),
        ("query spaces", {"query": "  kick  "}), ("priority", {"priorities": ["hoch", "niedrig"]}),
        ("tags", {"tags": ["Vergabe"]}), ("assignee", {"assignees": ["u3"]}),
        ("columns", {"columns": ["offen"]}), ("overdue", {"due_states": ["overdue"]}),
    ...
    for name, raw in filters:
        criteria = CardFilter(
            query=raw.get("query", ""),
            **{k: frozenset(raw.get(k, [])) for k in
               ("priorities", "tags", "assignees", "columns", "due_states")},
        )
...
def deadline_cases() -> list[Case]:
    values = [None, "", "2026-09-24", "2026-09-25", "2026-09-28", "2026-09-29",
              "2026-09-25T23:59:00", "2026-09-25T00:00:00Z", "kaputt", "2026-02-30"]
    return [case(f"due {d!r}", {"due": d, "today": "2026-09-25"},
                 deadline_state(d, date(2026, 9, 25))) for d in values]
        ...
        for user in ("u1", "u2", "u3", "u4", "u9"):
            for action in Action:
                ...
                cases.append(case(
                    ...
                    {"op": "authorize", "board": bj(board), "user_id": user,
                     "action": action.value, "inherited": inherited},
                    {"role": role_of(board, user, inherited), "decision": decision.to_json()},
                ))
    for actor, target, permission in (("u1", "u5", "read"), ("u1", "u5", "write"),
                                      ("u1", "u1", "edit"), ("u2", "u5", "read"),
                                      ("u9", "u5", "read"), ("u1", "u2", "read")):
        cases.append(case(f"share {actor}->{target} {permission}",
                          {"op": "share", "board": bj(board), "actor": actor,
                           "target_user": target, "permission": permission},
                          check_share(board, actor, target, permission).to_json()))
    for actor, target in (("u1", "u2"), ("u3", "u3"), ("u2", "u3"), ("u1", "u7")):
        cases.append(case(f"revoke {actor}->{target}",
                          {"op": "revoke", "board": bj(board), "actor": actor,
                           "target_user": target},
...
def validation_cases() -> list[Case]:
    fields: list[tuple[str, dict[str, Any]]] = [
        ("title ok", {"title": "  Titel  "}), ("title empty", {"title": "   "}),
        ...
        ("priority ok", {"priority": "hoch"}), ("priority bad", {"priority": "dringend"}),
        ("due date", {"due": "2026-09-30"}), ("due z", {"due": "2026-09-30T10:00:00Z"}),
        ("due bad", {"due": "30.09.2026"}), ("due empty", {"due": ""}),
        ("badge 20", {"badge": "B" * 20}), ("badge 21", {"badge": "B" * 21}),
        ("badge empty", {"badge": ""}), ("color empty", {"color": ""}),
        ("tags wrong type", {"tags": "VP"}), ("checklist", {"checklist": [{"text": "a",
                                                                         "done": True}]}),
    ]
    cases = [case(name, {"op": "card_fields", "fields": raw},
    ...
    column_sets: list[tuple[str, list[dict[str, Any]]]] = [
        ...
        ("label 81", [col("a", "L" * 81)]), ("wip 0", [col("a", wip_limit=0)]),
        ("wip 1", [col("a", wip_limit=1)]),
    ]
    cases += [case(name, {"op": "columns", "columns": raw},
                   _outcome(lambda raw=raw: [_jsonable_column(c) for c in
                                             v.validate_columns(columns_from_json(raw))]))
...
def command_cases() -> list[Case]:
    ...
    ops: list[tuple[str, dict[str, Any], Callable[[Board], Any]]] = [
        ("move to top", {"op": "move", "card_id": "b", "column_id": "offen", "index": 0},
         lambda b: move_card(b, ctx, "b", "offen", index=0)),
        ("move before", {"op": "move", "card_id": "d", "column_id": "offen", "before_id": "b"},
         lambda b: move_card(b, ctx, "d", "offen", before_id="b")),
        ("move after", {"op": "move", "card_id": "d", "column_id": "offen", "after_id": "b"},
         lambda b: move_card(b, ctx, "d", "offen", after_id="b")),
        ("move end default", {"op": "move", "card_id": "a", "column_id": "erledigt"},
         lambda b: move_card(b, ctx, "a", "erledigt")),
        ("move index clamp", {"op": "move", "card_id": "a", "column_id": "erledigt",
                              "index": 99}, lambda b: move_card(b, ctx, "a", "erledigt", index=99)),
        ("move negative", {"op": "move", "card_id": "d", "column_id": "offen", "index": -3},
         lambda b: move_card(b, ctx, "d", "offen", index=-3)),
        ("toggle done", {"op": "toggle_done", "card_id": "a"},
         lambda b: toggle_done(b, ctx, "a")),
        ("toggle reopen", {"op": "toggle_done", "card_id": "d"},
         lambda b: toggle_done(b, ctx, "d")),
        ("create end", {"op": "create", "fields": {"title": "Neu", "column_id": "offen"}},
         lambda b: create_card(b, ctx, {"title": "Neu", "column_id": "offen"})),
        ("create index", {"op": "create", "fields": {"title": "Neu", "index": 1}},
         lambda b: create_card(b, ctx, {"title": "Neu", "index": 1})),
        ("configure remove", {"op": "configure", "columns": [
            {"id": "offen", "label": "Offen"}, {"id": "erledigt", "label": "Erledigt"}]},
         lambda b: configure_columns(b, ctx, (Column("offen", "Offen"),
                                              Column("erledigt", "Erledigt")))),
    ]
    dup = _replace(board, "cards", tuple(
        _replace_card(c, "V") if c.column_id == "offen" else c for c in board.cards))
    cases = []
    for name, input_, fn in ops:
        result = fn(board)
        cases.append(case(name, {"board": bj(board), "actor": "u1", "now": NOW, "new_id": "new",
                                 **input_},
                          {"orders": _orders(result.board), "version": result.board.version}))
    rebalance = move_card(dup, ctx, "d", "offen", index=1)
    cases.append(case("move with duplicate ranks rebalances",
                      {"board": bj(dup), "actor": "u1", "now": NOW, "op": "move",
                       "card_id": "d", "column_id": "offen", "index": 1},
