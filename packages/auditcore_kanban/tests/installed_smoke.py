...
def main() -> None:
    ...
    created = api.handle("POST", "/boards", user_id="u1", body={"id": "b1", "title": "Prüfung",
                                                                 "template": "vorhabenpruefung"})
    assert created.status == 201
    card = api.handle("POST", "/boards/b1/cards", user_id="u1", body={"title": "Belege"})
    moved = api.handle("POST", f"/boards/b1/cards/{card.body['card']['id']}/move",  # type: ignore[index,call-overload]
                       user_id="u1", body={"column_id": "pruefung"})
    ...
    assert [c.title for c in service.find_cards("b1", "u1", CardFilter(query="beleg"))] == [
        "Belege"]
    legacy = import_workspace({"id": "p", "owner_user_id": 1},
