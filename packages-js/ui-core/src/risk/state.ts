...
/** Angaben der Regel für einen Eintrag; ohne Regel (unbekannter Code) neutrale Werte. */
function ruleFacts(code: string, rule: RuleView | undefined): Pick<FlagEntry, 'label' | 'severity' | 'note' | 'parameters' | 'origin'> {
  if (!rule) return { label: code, severity: null, note: null, parameters: {}, origin: {} }
  return { label: rule.label ?? code, severity: rule.severity ?? null, note: rule.note ?? null, parameters: rule.parameters ?? {}, origin: rule.origin ?? {} }
}

function undeterminedEntry(code: string, reason: string, rule: RuleView | undefined, inputs: JsonObject): FlagEntry {
  return { code, state: 'undetermined', reason, inputs, evidence: {}, messages: null, ...ruleFacts(code, rule) }
}
