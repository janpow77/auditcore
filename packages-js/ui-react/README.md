...
Abhängigkeitshülle: dazu `@flowaudit/ui-core` und `@flowaudit/common`; Peer-Abhängigkeiten `react` und `react-dom` ^18.3, kein Vue. Stile: `@flowaudit/ui-core/style.css`. Nur der veraltete Einstieg `@flowaudit/ui-react/elements` braucht zusätzlich `@flowaudit/ui`, `@flowaudit/kanban-core` und `vue`.
