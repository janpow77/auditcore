...
watch(
  () => props.open,
  (open) => {
    if (open) draft.value = cloneInfo(props.info)
  },
  { immediate: true },
)
...
const toggleFund = (code: string) => (draft.value = toggled(draft.value, code))
const listItems = (key: string) => infoList(draft.value, key)
</script>
