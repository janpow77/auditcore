...
export const PRIORITY_TONES: Readonly<Record<Priority, PriorityTone>> = { hoch: 'danger', mittel: 'warning', niedrig: 'success' }
...
/** Dateigröße als Zahl und Einheit (B, KB, MB, GB); die Zahl formatiert die Oberfläche je Sprache. */
export function fileSize(bytes: number): { value: number; unit: 'B' | 'KB' | 'MB' | 'GB' } {
  ...
  return { value, unit: units[unit] ?? 'GB' }
}

/** Inline-Stil einer Karte mit eigener Farbe bzw. Hintergrundbild (CSS-Variablen wie im Original). */
export function cardStyle(color: string | null, image: string | null): Record<string, string> {
  const result: Record<string, string> = {}
  if (color) {
    result['--fa-kanban-card-bg'] = color
    if (textOn(color) === 'light') result['--fa-kanban-card-fg'] = '#f8fafc'
  }
  if (image) result.backgroundImage = `linear-gradient(rgb(0 0 0 / 0.35), rgb(0 0 0 / 0.35)), url("${encodeURI(image)}")`
  return result
}

/** Größte Bilddatei für das Kartendesign (2 MB wie im Original). */
